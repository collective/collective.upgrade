...
class PortalUpgrader(utils.Upgrader):
    ...
    def upgrade(
        self, upgrade_portal=True, upgrade_all_profiles=True, upgrade_profiles=(), **kw
    ):
        ...
        self.log(f"Upgrading {self.context}")
        ...
        if upgrade_all_profiles:
            if upgrade_profiles:
                raise ValueError("upgrade_profiles conflicts with upgrade_all_profiles")
            upgrade_profiles = self.setup.listProfilesWithPendingUpgrades()
        if upgrade_profiles:
            self.upgradeExtensions(upgrade_profiles, **kw)

        self.log(f"Upgraded {self.context}")
    ...
    def doUpgrades(self, profile_id, steps_to_run):
        ...
        if steps_to_run:
            self.log(
                "Upgrading profile {!r} to {!r}".format(
        ...
        for info in steps_to_run:
            ...
            if step is not None:
                ...
                self.log(f"Running upgrade step for {msg}.")
                step.doStep(self.setup)
                self.log(f"Finished upgrade step for {msg}.")
        ...
        self.commit(
            "Upgraded profile {!r} to {!r}".format(profile_id, ".".join(step.dest))
        )
